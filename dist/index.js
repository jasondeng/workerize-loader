...
loader.pitch = function (request) {
  ...
  compilationHook(worker.compiler, function (compilation, data) {
    parseHook(data, function (parser) {
      exportDeclarationHook(parser, function (expr) {
        var decl = expr.declaration || expr;
        var _parser$state = parser.state,
            stateCompilation = _parser$state.compilation,
            current = _parser$state.current;
        var entryModule = stateCompilation.entries instanceof Map ? stateCompilation.moduleGraph.getModule(stateCompilation.entries.get(bundleName).dependencies[0]) : stateCompilation.entries[0]; // only process entry exports
